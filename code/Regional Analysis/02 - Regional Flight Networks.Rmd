...
for (i in 1:lengths(dests)) {
  ...
  points(
    ...
    pch=1, cex = 0.25, lwd = 0.4,
    col = viridis(lengths(dests), begin = 0.2, alpha = 0.6)[i]
    )
...
# Labels cities
 text(
   x = conventions[IATA %in% names(dest.cols.list) & IATA != "PHL"]$lon,
   y = conventions[IATA %in% names(dest.cols.list) & IATA != "PHL"]$lat,
   labels = conventions[IATA %in% names(dest.cols.list) & IATA != "PHL"]$CITY,
   col = "white", cex = 0.10, pos = 3, offset = 0.10)
 
# Label PHL w/ lower offset
 text(
   x = conventions[IATA == "PHL"]$lon,
   y = conventions[IATA == "PHL"]$lat,
   labels = conventions[IATA == "PHL"]$CITY,
   col = "white", cex = 0.10, pos = 1, offset = 0.10)

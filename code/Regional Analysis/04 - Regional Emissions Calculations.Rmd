...
merge.test <- merge(nyc[ATTENDEE_STATUS == "Approved"],
  main.nyc[, .(location, lat, lon, Frequency)],
  by = c("location", "lat", "lon"), all = TRUE)
...
# 
# merge.test[Frequency.x != Frequency.y]$Frequency.x <- merge.test[Frequency.x != Frequency.y]$Frequency.y
# 
...
nyc.fix <- merge.test[, `:=`(
  ...
  Frequency.x = NULL
  )][, Frequency.y := NULL][!is.na(Frequency)]
...
# Merge to check, frequency
merge.test.sf <- merge(sf, main.sf[, .(location, lat, lon, Frequency)],
  by = c("location", "lat", "lon"), all = TRUE)
